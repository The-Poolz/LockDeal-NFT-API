...
{
    public static IProvider Create(string address, BigInteger poolId, byte decimals, params BigInteger[] values) =>
         Create(ProvidersAddresses[address], poolId, decimals, values);

    public static IProvider Create(ProviderName name, BigInteger poolId, byte decimals, params BigInteger[] values) =>
        Providers(poolId, decimals, values)[name];

    public static Dictionary<ProviderName, IProvider> Providers(BigInteger poolId, byte decimals, params BigInteger[] values) => new()
    {
